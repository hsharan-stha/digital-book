        ...
        @forelse ($pages as $page)
            <div class="container ">
                <div id="bookmark">
                    ...
                    <div id="unmark-{{$page->pageno}}" class="">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="size-6">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M17.593 3.322c1.1.128 1.907 1.077 1.907 2.185V21L12 17.25 4.5 21V5.507c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0 1 11.186 0Z" />
...
<script type="text/javascript">
  ...
  // Remove any previous handlers for cleanliness
  $flip.off("click.bookmarks");

  // Delegate clicks for both mark-X (remove) and unmark-X (add)
  $flip.on("click.bookmarks", '[id^="mark-"], [id^="unmark-"]', async function (e) {
    const id = this.id || "";
    const m = id.match(/^(unmark|mark)-(.+)$/);
    if (!m) return;

    const action = m[1];                // "unmark" (add) or "mark" (remove)
    const pageStr = String(m[2]);       // keep page as string consistently
const visible = new Set(Array.from($flip.turn("view") || []).map(String));
if (!visible.has(pageStr)) return;
    try {
      if (action === "mark") {
        // Remove bookmark
