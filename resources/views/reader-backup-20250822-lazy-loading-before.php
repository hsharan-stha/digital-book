...
`;
                    ...
                    iconWrapper.onclick = async (e) => {
                        ...
                        if (confirm(`Are you sure you want to delete bookmark on page ${item.page}?`)) {
                            ...
                            // Build a quick lookup set for existing bookmarks
        ...
        // Bookmark buttons
          function addEventListenerInBookMark() {
            ...
            // Debounce guard to prevent double triggers
            let busy = false;

            // Delegate for both add/remove controls
            const selector = '[id^="mark-"], [id^="unmark-"]';

            // Prefer pointer events for reliable touch handling
            $flip.on("pointerup.bookmarks", selector, async function (e) {
                ...
                // Prevent ghost clicks / text selection / focus scroll
                e.preventDefault();
                e.stopPropagation();
                ...
                const action  = m[1];           // "unmark" (remove) or "mark" (add)
                const pageStr = String(m[2]);   // keep page as string consistently

                // ---- Improved visible check (handles single + two-page spreads) ----
                const cur   = String($flip.turn("page"));
                const view  = ($flip.turn("view") || []).map(String);
                const isVisible = pageStr === cur || view.includes(pageStr);
                if (!isVisible) return;

                // Ensure the page exists in DOM (some themes lazy-load offscreen pages)
                const $page = $flip.find(`[page="${pageStr}"]`);
                if ($page.length === 0) return;

                // In case bookmarksArray was mutated elsewhere, resync the Set quickly
                ...
                // UI refresh (batch to next frame to keep taps snappy)
                ...
                // Small timeout prevents rapid double-tap retriggers on iOS
