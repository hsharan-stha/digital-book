...
<script type="text/javascript">
    document.addEventListener("DOMContentLoaded", async function() {
        ...
        const facepages = parseInt("{{ $pageNumberDetails[0] }}");
        const lastpagenumber = parseInt("{{ $pageNumberDetails[1] }}");

        console.log(facepages,lastpagenumber)
        ...
        function setPageName(page) {
            ...
            // Handle back pages
            if (page > lastpagenumber + facepages) {
        ...
        for (let i = 1 - facepages; i <= (totalPages - facepages); i++) {
            const option = document.createElement("option");
            option.value = i + facepages;
            option.textContent = `${setPageName(i + facepages)} ページ`;
